...
/**
 * \brief Set PWM_ALT output & pull configuration
 *
 * Configure pin to pull up, down or disable pull mode, supported pull
 * modes are defined by device used
 *
 * \param[in] output_pull_mode Pin output & pull mode
 */
static inline void PWM_ALT_set_output_pull_mode(const enum port_output_pull_mode output_pull_mode)
{
	PORTD_set_pin_output_pull_mode(3, output_pull_mode);
}

/**
 * \brief Set PWM_ALT data direction
 *
 * Select if the pin data direction is input, output or disabled.
 * If disabled state is not possible, this function throws an assert.
 *
 ...
 *                      (low power state)
 */
static inline void PWM_ALT_set_dir(const enum port_dir dir)
{
	PORTD_set_pin_dir(3, dir);
}

/**
 * \brief Set PWM_ALT input/sense configuration
 *
 * Enable/disable PWM_ALT digital input buffer and pin change interrupt,
 * select pin interrupt edge/level sensing mode
 *
 * \param[in] isc PORT_ISC_BOTHEDGES_gc     = Sense Both Edges
 *                PORT_ISC_RISING_gc        = Sense Rising Edge
 ...
 *                PORT_ISC_LEVEL_gc         = Sense low Level
 */
static inline void PWM_ALT_set_isc(const PORT_ISC_t isc)
{
	PORTD_pin_set_isc(3, isc);
}

/**
 * \brief Set PWM_ALT inverted mode
 *
 * Enable or disable inverted I/O on a pin
 *
 * \param[in] inverted true  = I/O on PWM_ALT is inverted
 *                     false = I/O on PWM_ALT is not inverted
 */
static inline void PWM_ALT_set_inverted(const bool inverted)
{
	PORTD_pin_set_inverted(3, inverted);
}

/**
 * \brief Set PWM_ALT level
 *
 * Sets output level on a pin
 *
 * \param[in] level true  = Pin level set to "high" state
 *                  false = Pin level set to "low" state
 */
static inline void PWM_ALT_set_level(const bool level)
{
	PORTD_set_pin_level(3, level);
}

/**
 * \brief Toggle output level on PWM_ALT
 *
 * Toggle the pin level
 */
static inline void PWM_ALT_toggle_level()
{
	PORTD_toggle_pin_level(3);
}

/**
 * \brief Get level on PWM_ALT
 *
 * Reads the level on a pin
 */
static inline bool PWM_ALT_get_level()
{
	return PORTD_get_pin_level(3);
}

/**
 * \brief Set PWM_ALT interrupt level
 *
 * Sets interrupt level for port
 *
 * \param[in] level Value to write to the port register
 */
static inline void PWM_ALT_int_level(const uint8_t level)
{
	PORTD_set_int_level(level);
}

/**
 * \brief Set PWM_ALT interrupt vector 0 mask
 *
 * Sets interrupt mask for port vector 0
 *
 * \param[in] value Value to write to the port register
 */
static inline void PWM_ALT_int0_mask(const uint8_t value)
{
	PORTD_write_int0_mask(value);
}

/**
 * \brief Set PWM_ALT interrupt vector 1 mask
 *
 * Sets interrupt mask for port vector 1
 *
 * \param[in] value Value to write to the port register
 */
static inline void PWM_ALT_int1_mask(const uint8_t value)
{
	PORTD_write_int1_mask(value);
}
