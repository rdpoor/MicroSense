...
#define RESET_HI_THRESH 0.8
#define RESET_LO_THRESH 0.3
...
static void led_on();
static void led_off();
static void process_reset_enable(float ratio);
...
static void reset_integrator() {
  ...
  // xxx nope! Assert A and B simultaneously
...
static void process_reset_enable(float ratio) {
	if (ratio > RESET_HI_THRESH) {
		TP_0_set_level(1);  // aka RESET_EN
	} else if (ratio < RESET_LO_THRESH) {
		TP_0_set_level(0);  // aka RESET_EN
	}
}
