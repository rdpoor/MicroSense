...
// Called whenever the ADC has converted a sample.
void micro_sense_sample_cb(void) {
  PROFILING_SET(PROCESSING_SAMPLE);
  process_sample();
  PROFILING_CLR(PROCESSING_SAMPLE);
}
...
// mSec)
static void process_sample(void) {
  float ratio = ADC_COUNT_TO_RATIO(ADC_0_get_conversion_result());
  float d_ratio;
